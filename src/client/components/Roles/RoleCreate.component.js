...
const RoleCreate = () => {
    ...
    const getRoleInfo = () => {
        http.patch('role', null, { _id: _id }, null)
            .then(
                role => {
                    setCode(role.data.code)
                    setDescription(role.data.description)
                    role.data.expirationDate && setExpirationDate(moment(role.data.expirationDate).format('YYYY-MM-DD'))
    ...
    const handleSubmit = () => {
        if (action == 'Crear') {
            http.post('role', { code: code, description: description, expirationDate: expirationDate != '' ? expirationDate : undefined, appId: appId }, null, null)
                .then(
                    resp => {
            ...
            http.put('role',
                { code: code, description: description, expirationDate: expirationDate != '' ? expirationDate : undefined, appId: appId },
                { _id: _id },
                null)
                .then(
                    resp => {
